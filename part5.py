...
class AStar(object):
    def __init__(self, grid_width=101, grid_height=101):
        ...
        self.expanded = set()  # new added for adaptive A*
        self.goal_distance = {} # new added for adaptive A*

    # new added for adaptive A*
    def get_expanded_cells(self):
    ...
    def solve(self):
        ...
        while len(self.opened):
            ...
            self.expanded.add(cell)  # new added for adaptive A*
            if cell is self.end:
                # find the path
...
class AdaptiveAStar(AStar):
    ...
    def update_heuristic(self):
        for cell in self.get_expanded_cells():
            self.goal_distance[cell] = self.goal_distance[self.end] - cell.g

    # get h
    def get_heuristic(self, cell):
        if cell in self.goal_distance:
            return self.goal_distance[cell]
        else:
            return super().get_heuristic(cell)
...
for i in range(1, 51):
    ...
    # initialize grid using filename
    adaptive.init_grid(filename)
    # only calculate the time for solving
    start_time = time.time()
    adaptive.solve()
    end_time = time.time()
    time_backward = end_time - start_time
    total_time_adaptive += time_backward

print("Total time for repeated forward A*: ", total_time_forward)
print("Total time for adaptive A*: ", total_time_adaptive)
...
adaptive.init_grid(filename)
p2 = adaptive.solve()
grid_adaptive = adaptive.get_grid()
visualize_grid(axs[1], grid_adaptive, p2)
axs[1].set_title('Adaptive A*')
